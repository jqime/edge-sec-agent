...
Incluye: historial SQLite, informe HTML con gráfico 7 días, API de historial, etc.
"""
import json, sqlite3, subprocess, sys, os, urllib.request, time, re
...
if os.path.exists(SECRETS_PATH):
    try:
        with open(SECRETS_PATH) as f: exec(f.read(), globals())
    except: pass
...
def _insert_history(timestamp, score, ssh, ports, temp, ram):
    _ensure_history_db()
    conn = sqlite3.connect(DB_PATH); cur = conn.cursor()
    cur.execute("INSERT INTO history (timestamp, score, ssh_failures, open_ports, temp_c, ram_percent) VALUES (?,?,?,?,?,?)", (timestamp, int(score), int(ssh), ports, float(temp) if temp else None, float(ram) if ram else None))
    conn.commit(); conn.close()
...
def _get_last_days_scores(days=7):
    ...
    return [{"date": day, "score": int(avg_score) if avg_score else 0} for day, avg_score in rows]
...
def _count_ssh_failures():
    paths = os.environ.get("SSH_LOG_PATHS", "/var/log/auth.log,/var/log/secure").split(","); total=0
    for p in paths:
        if os.path.exists(p):
            try:
                with open(p, "r", errors="ignore") as f: total += sum(1 for line in f if "Failed password" in line or "authentication failure" in line.lower())
            except: pass
    return total
                ...
                for line in out.splitlines():
                    if "LISTEN" in line:
                        for token in line.split():
                            if ":" in token: port = token.split(":")[-1]
                            if port.isdigit(): ports.add(port)
                if ports: return sorted(ports, key=int)
        except: pass
    try:
...
def _get_temperature_c():
    for i in range(10):
        ...
        if os.path.exists(path):
            try:
                with open(path,"r") as f: return int(f.read().strip())/1000.0
            except: continue
    return None
...
def _get_ram_usage_percent():
    try:
        with open("/proc/meminfo","r") as f:
            ...
            if total and avail: return ((total-avail)/total)*100.0
    except: pass
    return None
...
def _format_report(score, ssh, ports, temp, ram):
    ...
    if not (ssh or ports or (temp and temp>55) or (ram and ram>60)): lines.append("  • Sistema estable. Mantén monitorización.")
    return "\n".join(lines)

def _format_html_report(score, ssh, ports, temp, ram, ts=None, recs=None, history7=None):
    ...
    rec_html = ""
    if recs:
        rec_html = "<h3>Recomendaciones</h3><ul>" + "".join(f"<li>{r}</li>" for r in recs) + "</ul>"
    history7_js = "[]"
    history7_html = ""
    if history7:
...
def _generate_html_recommendations(ssh, ports, temp, ram):
    recs = []
    if ssh>0: recs.append("SSH: usar autenticación por clave, deshabilitar root y login por contraseña, activar fail2ban.")
    if ports: recs.append("Puertos abiertos: cerrar servicios no esenciales, usar firewall.")
    if temp and temp>55: recs.append("Temperatura alta: mejorar refrigeración.")
    if ram and ram>60: recs.append("RAM alta: optimizar procesos, añadir swap.")
...
def save_html_report(html_text):
    ...
    with open(path, "w", encoding="utf-8") as f: f.write(html_text)
    return path
...
def cmd_html():
    ...
    html = _format_html_report(score, ssh, ports, temp, ram, timestamp=None, recs=recs, history7=history7)
...
def main():
    args = sys.argv[1:]
    if "--html" in args: return cmd_html()
    if "--security" in args: return cmd_security(json_output="--json" in args)
